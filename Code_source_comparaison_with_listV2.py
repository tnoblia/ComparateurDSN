...
class Table_DSN():
    ...
    def creer_dic_rubriques(self):
        ...
        for ligne in self.liste_lignes:
            try:
                dic_rubrique[ligne.individu].append(ligne)
            except:
                dic_rubrique[ligne.individu] = [ligne]
        return dic_rubrique

    def creer_dic_rubrique_individu(self):
        dic_rubrique = {}
        for ligne in self.liste_lignes:
            try:
                dic_rubrique[ligne.individu][ligne.rubrique].append(ligne)
            except:
                try:
                    dic_rubrique[ligne.individu][ligne.rubrique] = [ligne]
                except:
                    dic_rubrique[ligne.individu] = {ligne.rubrique : [ligne]}
        return dic_rubrique
    ...
    def differences(self, other):
        ...
        def bar_c():
            ...
            dicoA = self.creer_dic_rubriques()
            dicoB = other.creer_dic_rubrique_individu()
            list_diff1 = []
            list_diff2 = []
            for individu in dicoA.keys():
                try:
                    dicoB_analyse = dicoB[individu]
                except:
                    dicoB_analyse = {}
                for ligneDSN in dicoA[individu]:
                    try:
                        dicoB_analyse[ligneDSN.rubrique].remove(ligneDSN)
                    except:
            ...
            for individu in dicoB.keys():
                for rubrique in dicoB[individu].keys():
                    for ligneDSN in dicoB[individu][rubrique]:
                        lib_bloc = df_DSN['Libellé bloc'][df_DSN['id bloc'] == ligneDSN.bloc].values[0]
                        lib_rub = df_DSN['Libellé rubrique'][df_DSN['Id rubrique'] == ligneDSN.rubrique].values[0]
